...
const JobListing = () => {
  ...
  function handleCategoryChange(category ) {
    ...
    useEffect(() => {
      let matchesCategory = job => selectedCategory.length === 0 || selectedCategory.includes(job.category)
      let matchesLocation = job => selectedLocation.length === 0 || selectedLocation.includes(job.location)

      let matchesSearchTitle = job => searchFilter.jobTitle === "" || job.jobTitle.toLowerCase().includes(searchFilter.jobTitle.toLowerCase())
      let matchesSearchLocation = job => searchFilter.location === "" || job.location.toLowerCase().includes(searchFilter.location.toLowerCase())

      let filtered = jobList.filter(matchesCategory).filter(matchesLocation).filter(matchesSearchTitle).filter(matchesSearchLocation)
  ...
  return (
    <div className="container mx-auto flex flex-col lg:flex-row xl:px-14 px-10 ">
      <div className="w-full lg:w-1/4 p-5 border">
        ...
        <button
          className="lg:hidden mb-4 bg-blue-500 text-white px-4 py-2 rounded-full font-semibold hover:bg-blue-700 transition-all duration-300"
          ...
          <ul className="flex flex-col gap-2">
            {jobCategories.map((category, index) => (
              <li key={index} className="text-md font-normal cursor-pointer">
                <input type="checkbox" id={category} className="mr-2 scale-110" onChange={() => handleCategoryChange(category)} />
        ...
        >
          ...
          <ul className="flex flex-col gap-2">
            {jobLocations.map((location, index) => (
              <li key={index} className="text-md font-normal cursor-pointer">
                <input type="checkbox" id={location} className="mr-2 scale-110" onChange={() => handleLocationChange(location)} />
      ...
      <div className="w-full lg:w-3/4 px-4 mt-10 lg:mt-0">
        <h1 className="text-3xl py-2 font-bold" id="latest-jobs">Latest Jobs</h1>
        <p className="text-md mb-8 text-gray-500">
          Choose jobs from the top employers and apply for the same.
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredJobList.slice((currentPage - 1) * 6, currentPage * 6).map((job, index) => (
        ...
        </div>
      {
        jobList.length > 0 && (
          <div className="flex justify-center gap-2 items-center my-10">
            <a href="#latest-jobs" onClick={() => setCurrentPage(Math.max(1, currentPage - 1))}>
              <ChevronLeft className="size-6 cursor-pointer text-blue-500 hover:text-blue-700" />
            </a>
            {
              Array.from({length: Math.ceil(jobList.length / 6)}, (_, index) => (
                <a href="#latest-jobs" onClick={() => setCurrentPage(index + 1)}>
                  <button key={index} className={`${currentPage === index + 1 ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-500"} px-3 p-1 rounded font-semibold hover:bg-blue-700 transition-all duration-300`}>
                  {index + 1}
                </button>
                </a>
              ))
            }
            <a href="#latest-jobs" onClick={() => setCurrentPage(Math.min(Math.ceil(jobList.length / 6), currentPage + 1))}>
